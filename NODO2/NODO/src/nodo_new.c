...
#include "nodo_new.h"
#include <stdlib.h>
#define PATH_CONFIG "/home/daniel/tp-2015-1c-the-byteless/NODO/NODO.config"
...
void connectToFileSistem(int* sock) {
	...
	*/
	*sock = new_connection(IP_FS, PUERTO_FS);
...
char *obtenerRutaArchivoTemporal(char *nombreArhivo);
void freeMensaje_t(mensaje_t *mensaje);
void *fs_nodo_conection_handler(void* ptr){
	...
	char** result;
	char* bloque;
	buffer_send = malloc(sizeof(mensaje_t));
	buffer_recv = malloc(sizeof(mensaje_t));
	int n_bloque = 0;
	...
	while(1) {
		...
		result = string_split(buffer_recv->comando, " ");
			...
			if (strcmp(result[0], "getBloque") == 0) {
				...
				n_bloque = atoi(result[1]);
				bloque = getBloque(n_bloque);  //getBloque bloque
				buffer_send->comandoSize = 1;
				buffer_send->comando = "\0";
				buffer_send->dataSize = TAMANIO_BLOQUE;
				buffer_send->data = bloque;
				enviar(sockFD, buffer_send);
				freeMensaje_t(buffer_recv);
				freeMensaje_t(buffer_send);
				freeMemory(result);

			}
			if (strcmp(result[0], "setBloque") == 0) {   //setBloque bloque [datos]
				n_bloque = atoi(result[1]);
				setBloque(n_bloque, buffer_recv->data);
				freeMensaje_t(buffer_recv);
				freeMemory(result);
			}
			if (strcmp(result[0], "getFileContent") == 0) { //getFileContent nombre
				t_fileContent* fileContent;
				fileContent = getFileContent(obtenerRutaArchivoTemporal(result[1]));
				buffer_send->comandoSize = 1;
				buffer_send->comando = "\0";
...
char *obtenerRutaArchivoTemporal(char *nombreArhivo){
	char* rutaArchivo = malloc(50);
	sprintf(rutaArchivo,"%s/%s", DIR_TEMP, nombreArhivo);
	return rutaArchivo;
}
