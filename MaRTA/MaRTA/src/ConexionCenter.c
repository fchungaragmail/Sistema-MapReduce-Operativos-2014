...
void *listenFS()//ya esta en un hilo de MaRTA
{
	...
	recv(conexionNueva.sockfd,buffer, BUFFER_SIZE, 0);
	return buffer;
}
...
void *listenJobs() //ya esta en un hilo de MaRTA
{
	...
	while (1) {
		...
		conexionNueva->sockfd = nuevoSocketfd;
		conexionNueva->thread = hiloCliente;
		pthread_create(&hiloCliente, NULL, jobHandler,(void*) conexionNueva);
		// lo 1ero q me pasa es la direccion de su arch. a procesar
		list_add(listaConexiones, (void*) conexionNueva);

		//ACA PUEDE QUE TENGA Q IR EL "SELECT" Y PUEDO DEVOLVER A MARTA DIRECTAMENTE

	}
}

void* jobHandler(void* conexionCliente) {
	Conexion conexion = *((Conexion*) conexionCliente);
	char buffer[BUFFER_SIZE];

	while (1) {
		recv(conexion.sockfd, buffer, BUFFER_SIZE, 0);
		//CADA RESPUES QUE LLEGA AL recv LE DEBERIA LLEGAR A MaRTA !!
		BroadCast(buffer);

		jobMessage(buffer);//funcion en .c de MaRTA
	}
